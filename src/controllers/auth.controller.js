...
export const signUp = async(req, res) =>{ 
    try {
        const { nombre, apellido, rut, telefono, correo, contraseña } = req.body
        ...
        if(verifyUser) return res.status(500).json({ message: 'El rut ingresado ya tiene una cuenta', status: 500 })
            ...
            const contraseñaEncrypt = await bcrypt.hash(contraseña, 10)
...
export const login = async(req, res) => {
    try {
        ...
        validateLoginData(verifyUserByCorreo, contraseña)       
...
export const verifyUser = async(req, res) => {
   try {
      const user = await User.findById(req.data.id).select("-contraseña")
       res.json(user);
   } catch (error) {
       return res.status(500).json({message: "No pudimos verificar a este usuario"})
    }
}